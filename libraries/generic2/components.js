...
prx.types.basic_buttongroup = {
    ...
    ,onDisplay: function(item,containerid,symbol) {
        ...
        if(item.maskEnabled && $.browser.webkit) {
            cR += '#'+_id+' .icon {background-color: '+prx.componentsHelper.getProp(item.maskInactive,'color-background')+'; -webkit-mask-size: auto '+(item.iconSize*20)+'%;}';
            if (item.iconpos !== 'top'){
        ...
        $.each(item.buttons, function(i,elm) {
            ...
            cR += '<input type="radio" name="'+_id+'-radio-input" id="'+_id+'-radio-'+i+'" '+((prx.componentsHelper.getProp(item.selected,'num-other') == i || (item.selected.includes(JSON.stringify(i)))) ? 'checked' : '')+' data-role="none" '+((prx.componentsHelper.getProp(item.active,'num-other') == i) ? 'class="liveUpdate-activeBackgroundColor-background-color liveUpdate-activeTextColor-color changeProperty-activeBackgroundColor"' : '')+' ' +((!prx.componentsHelper.getProp(item.changeActive,'boolean')) ? 'disabled' : '')+ '>';
            cR += '<label for="'+_id+'-radio-'+i+'" '+((prx.componentsHelper.getProp(item.selected,'num-other') == i) || (item.selected.includes(JSON.stringify(i))) ? 'class="changeProperty-activeBackgroundColor liveUpdate-activeBackgroundColor-background-color liveUpdate-activeTextColor-color"' : '')+' >';
            if( prx.componentsHelper.getProp(item.iconpos,'other') != 'noicon' && prx.componentsHelper.getProp(item.iconpos,'other') != 'none') {
